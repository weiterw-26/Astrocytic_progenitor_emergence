...
def SMD(X,k_guess, trials = None,n_sub = None,prop_algo = 'agglo', class_algo = 'entropy', cluster_prior_min=None, z_score = True):
	N = X.shape[0]
	D = X.shape[1]
...
def find_classifier_dims_entropy(X,cell_sample,k_sub, cluster_algo):
	...
	gnout = [one_tree(Xit,k_guess,ikk1,ikk2) for ikk1 in np.unique(k_guess) for ikk2 in np.unique(k_guess) if ikk1<ikk2 and np.all([np.sum(k_guess==ikk1)>2, np.sum(k_guess==ikk2)>2])] # added last logical AND to remove small clusters
	return gnout
...
def loader(name, preprocess=False):
	if os.path.exists(f'{name}.npz'):
		X = np.array(load_npz(f'{name}.npz').todense())

	elif os.path.exists(f'{name}.npy'):
		X = np.load(f'{name}.npy', allow_pickle=True)
...
def preprocessor(X):
	...
	reads = np.sum(Xc, axis = 0)
	null_filt = reads!=0
	Xc = Xc[:, null_filt]
	null_filt = np.sum(Xc, axis =1) != 0
	Xc = Xc[null_filt, :]
	reads = np.sum(Xc, axis = 1)
	cutoff_filt = reads > np.percentile(reads, 30)
	Xc = Xc[cutoff_filt, :]
	
	 
	Xc =np.transpose(np.transpose(Xc)/(np.sum(Xc, axis = 1)))
	std_filt = np.std(Xc, axis = 0) > np.percentile(np.std(Xc, axis=0), 65)
	Xc = Xc[:, std_filt]
	mean_filt = np.mean(Xc, axis=0) > np.percentile(np.mean(Xc, axis=0), 30)
	Xc = Xc[:, mean_filt]
	Xc = np.log1p(Xc)
	Xc = zscore(Xc, axis = 0) 
	

	np.save('shape.npy', Xc.shape)
	return Xc	
